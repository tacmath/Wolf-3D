...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/02/16 17:20:11 by lperron           #+#    #+#             */
/*   Updated: 2020/02/16 17:22:59 by lperron          ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
unsigned int	*size_down(unsigned int *img_old, t_point old_siz, double ratio)
{
	unsigned int	*new;
	t_point			new_size;
	double			incr_new;
	...
	new_size = init_point(old_siz.x * ratio, old_siz.y * ratio);
	if (!(new = ft_memalloc(sizeof(unsigned int) * new_size.x * new_size.y)))
		return (NULL);
	incr_new = ratio;
	i.y = -1;
...
unsigned int	*cut_img(unsigned int *img_old, t_point old_size,
